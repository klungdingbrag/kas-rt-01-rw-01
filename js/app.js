...
function errorToast(err){ console.error(err); setDbStatus(false, err?.message || err); toast('❌ ' + (err?.message || err)); }
function setDbStatus(ok, message){ const el=$('connectionStatus'); if(!el)return; el.className='connection-pill '+(ok?'online':'offline'); el.innerHTML=`<span class="status-icon">${ok?'●':'●'}</span><span class="status-text">${esc(message || (ok?'Database tersambung':'Database tidak tersambung'))}</span>`; }
function navigate(page){ document.querySelectorAll('.page').forEach(p=>p.classList.remove('active')); document.querySelectorAll('.nav-item').forEach(b=>b.classList.toggle('active', b.dataset.page===page)); $('page-'+page).classList.add('active'); $('sidebar').classList.remove('open'); if(page==='transactions')renderTransactions(); }
function setNow(){ const d=new Date(); const local=new Date(d.getTime()-d.getTimezoneOffset()*60000).toISOString(); $('tanggal').value=local.slice(0,10); $('waktu').value=local.slice(11,16); }
function calculateDashboardFromServerTransactions(){
  const c=state.config||{};
  const rows=Array.isArray(state.transactions)?state.transactions:[];
  const active=rows.filter(t=>String(t.status||'').toUpperCase()==='AKTIF');
  let pemasukan=0,pengeluaran=0;
  active.forEach(t=>{
    const n=Number(t.nominal)||0;
    if(String(t.jenis)==='Pemasukan') pemasukan+=n;
    else if(String(t.jenis)==='Pengeluaran') pengeluaran+=n;
  });
...
$('editForm').addEventListener('submit',async e=>{e.preventDefault();try{const current=state.transactions.find(t=>String(t.id)===String($('editId').value));await KasApi.updateTransaction({password:$('editPassword').value,transactionId:$('editId').value,reason:$('editReason').value.trim(),data:{tanggal:$('eTanggal').value,waktu:$('eWaktu').value,jenis:current.jenis,kategori:$('eKategori').value,nominal:Number($('eNominal').value),keterangan:$('eKeterangan').value.trim(),catatan:$('eCatatan').value}});closeModal('editModal');toast('✓ Perubahan tersimpan di database.');await reload();}catch(err){errorToast(err)}});
