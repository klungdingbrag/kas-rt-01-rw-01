const KasApi = (() => {
  ...
  function request(action, options = {}) {
    ...
    if (!url || window.KASRT_CONFIG.demoMode) {
      return Promise.resolve(mock(action, options));
    }
    ...
    return fetch(target, fetchOptions).then(async r => {
      const text = await r.text();
      let data; try { data = JSON.parse(text); } catch { throw new Error('Backend tidak mengembalikan JSON yang valid.'); }
      if (!r.ok || data.success === false) throw new Error(data.message || 'Request gagal.');
      return data;
    });
  }
  ...
  function mock(action) {
    ...
    if (action === 'createTransaction') {
      const d = arguments[1]?.body || {};
      const item = { id:`TRX-DEMO-${Date.now()}`, timestamp:new Date().toISOString(), ...d, nominal:Number(d.nominal), status:'AKTIF' };
      demo.transactions.push(item); return {success:true,message:'Transaksi berhasil disimpan.',id:item.id};
    }
